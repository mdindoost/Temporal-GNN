#!/usr/bin/env python3
"""
Fixed Temporal Memory Module Implementation
Addresses dimension mismatch issues in temporal attention
"""
...
class NodeMemoryModule(nn.Module):
    """TGN-inspired node memory module with fixed dimensions"""
...
class GraphMemoryModule(nn.Module):
    ...
    def extract_graph_features(self, node_features: torch.Tensor, 
        ...
        node_std = torch.std(node_features, dim=0) if num_nodes > 1 else torch.zeros_like(node_mean)
        
        # Degree statistics
        if edge_index.shape[1] > 0:
            degrees = torch.bincount(edge_index[0], minlength=num_nodes).float()
            degree_mean = torch.mean(degrees)
            degree_std = torch.std(degrees) if num_nodes > 1 else torch.tensor(0.0, device=node_features.device)
        else:
            degree_mean = torch.tensor(0.0, device=node_features.device)
            degree_std = torch.tensor(0.0, device=node_features.device)
        ...
        basic_stats = torch.tensor([degree_mean, degree_std, density_tensor], device=node_features.device)
    ...
    def compute_graph_deviation(self, current_graph_features: torch.Tensor) -> torch.Tensor:
        ...
        return torch.clamp(deviation_score, 0.0, 10.0)  # Clamp for stability

class TemporalGCNEncoder(nn.Module):
    """Fixed temporal GCN encoder with proper dimension handling"""
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, 
        ...
        self.memory_projection = nn.Linear(hidden_dim, output_dim)  # Assume memory_dim = hidden_dim
...
class TrajectoryPredictor(nn.Module):
    """JODIE-inspired trajectory predictor with fixed dimensions"""
...
class TemporalAnomalyMemory:
    """Fixed unified temporal memory system"""
...
# Testing function
def test_fixed_temporal_memory():
    """Test the fixed temporal memory system"""
    print("Testing Fixed Temporal Memory System...")
    ...
    try:
        ...
        print(f"Node memory scores shape: {results['node_memory_scores'].shape}")
